...
Postgres + db-init + app, minden egyben:
...
#### Bundle mód (postgres + app, SQL-alapú init)

Ideális, ha előre generált SQL fájlból szeretnéd inicializálni az adatbázist, db-init konténer nélkül:

```bash
# Init SQL generálása (csak egyszer, vagy sémaváltozás után)
bun db:generate-sql

# Postgres + app indítása
bun docker:up:bundle
# vagy
docker compose -f docker/docker-compose.bundle.yml up -d
```

Az `init.sql` automatikusan betöltődik az első indításkor. Teljes reset: `docker compose -f docker/docker-compose.bundle.yml down -v`.

#### Csak app (külső adatbázis)

Ha saját PostgreSQL példányt kezelsz:
...
A `DATABASE_URL` változót a `.env`-ben a saját adatbázisodra kell állítani. A migrációkat és seed-et kézzel kell elvégezni.
